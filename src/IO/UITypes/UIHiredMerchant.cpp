...
namespace ms
{
		...
		item_count(0), selected_slot(-1), is_owner(false)
	{
		...
		buttons[Buttons::BT_OPEN] = std::make_unique<MapleButton>(src["BtStart"]);

		select_tex = src["select"];
		soldout_tex = src["soldout"];

		buttons[Buttons::BT_BUY]->set_active(false);
		buttons[Buttons::BT_ARRANGE]->set_active(false);
		buttons[Buttons::BT_CLOSE]->set_active(false);
		buttons[Buttons::BT_OPEN]->set_active(false);

		for (int i = 0; i < MAX_ITEMS; i++)
			items[i] = { 0, 0, 0, false };

		owner_label = Text(Text::Font::A12B, Text::Alignment::CENTER, Color::Name::BLACK, "");
		...
		chat_field.set_enter_callback(
			[this](std::string msg)
			{
				if (!msg.empty())
				{
					...
					if (is_owner)
						add_chat(Stage::get().get_player().get_name() + " : " + msg, 0);
		...
		meso_label = Text(Text::Font::A11M, Text::Alignment::LEFT, Color::Name::BLACK);
	...
	bool UIHiredMerchant::send_icon(const Icon& icon, Point<int16_t> cursorpos)
	{
		if (!is_owner)
			return true;
	...
	void UIHiredMerchant::set_slot_look(int8_t slot, const LookEntry& entry, const std::string& name)
	{
		if (slot < 0 || slot > 3)
			return;
	...
	void UIHiredMerchant::remove_visitor(int8_t slot)
	{
		if (slot >= 1 && slot <= 3)
		{
	...
	void UIHiredMerchant::rebuild_item_slider()
	{
	...
	void UIHiredMerchant::rebuild_chat_slider()
	{
	...
	void UIHiredMerchant::add_chat(const std::string& line, int8_t speaker)
	{
		Color::Name color;
	...
	Cursor::State UIHiredMerchant::send_cursor(bool clicked, Point<int16_t> cursorpos)
	{
	...
	void UIHiredMerchant::set_mode(bool owner, bool first_time)
	{
		is_owner = owner;
		buttons[Buttons::BT_BUY]->set_active(!owner);
		buttons[Buttons::BT_ARRANGE]->set_active(owner);
		buttons[Buttons::BT_OPEN]->set_active(owner && first_time);
		buttons[Buttons::BT_CLOSE]->set_active(owner && !first_time);
		buttons[Buttons::BT_EXIT]->set_active(!owner || !first_time);
	}

	void UIHiredMerchant::set_meso(int64_t meso)
	{
		meso_label.change_text(std::to_string(meso) + " meso");
	}

	void UIHiredMerchant::doubleclick(Point<int16_t> cursorpos)
	{
		if (!is_owner)
			return;
		...
		if (off.x() >= 5 && off.x() <= 195 && off.y() >= 186 && off.y() < 186 + 5 * 42)
		{
			...
			if (idx < item_count && !items[idx].sold_out)
				TakeItemBackPacket(idx).dispatch();
		}
	}

	void UIHiredMerchant::set_merchant(int32_t item_id)
	{
		std::string strid = "0" + std::to_string(item_id);
		employee = Animation(nl::nx::item["Cash"]["0503.img"][strid]["employee"]["say"]);
	}
	...
	void UIHiredMerchant::draw(float inter) const
	{
		...
		meso_label.draw(position + Point<int16_t>(126, 130));

		employee.draw(DrawArgument(position + Point<int16_t>(52, 127)), inter);
	...
	void UIHiredMerchant::send_scroll(double yoffset)
	{
	...
	void UIHiredMerchant::set_sold_out(int8_t slot)
	{
		if (slot >= 0 && slot < MAX_ITEMS)
	...
	Button::State UIHiredMerchant::button_pressed(uint16_t buttonid)
	{
		...
		case Buttons::BT_EXIT:
			if (is_owner)
				MerchantMaintOffPacket().dispatch();
			else
				TradeExitPacket().dispatch();
		...
		case Buttons::BT_CLOSE:
			CloseMerchantPacket().dispatch();
			deactivate();
			break;
		case Buttons::BT_OPEN:
		...
		case Buttons::BT_ARRANGE:
			MerchantOrganizePacket().dispatch();
			break;
