...
namespace ms
{
	namespace AiSkin
	{
		...
		{
			if (heldnode.data_type() != nl::node::type::bitmap ||
				donorpart.data_type() != nl::node::type::bitmap)
				return Texture();

			// Measure the donor frame's blade angle: principal axis of its
			// opaque pixels, signed so the blade points away from the grip
			nl::bitmap donorbmp = donorpart;
			int dw = donorbmp.width();
			int dh = donorbmp.height();

			if (dw <= 0 || dh <= 0)
				return Texture();

			// Grip in donor coords = origin + hand map point
			Point<int16_t> dorigin = donorpart["origin"];
			Point<int16_t> dmap;

			for (nl::node mapnode : donorpart["map"])
				if (mapnode.data_type() == nl::node::type::vector)
					dmap = mapnode;

			float grip_x = float(dorigin.x() + dmap.x());
			float grip_y = float(dorigin.y() + dmap.y());

			const uint8_t* dpx = static_cast<const uint8_t*>(donorbmp.data());
			...
			float axis_angle = 0.5f * std::atan2(float(2.0 * sxy), float(sxx - syy));
			float ax = std::cos(axis_angle);
			float ay = std::sin(axis_angle);

			// Blade points away from the grip
			float to_blade_x = float(cx) - grip_x;
			float to_blade_y = float(cy) - grip_y;
			...
			// Rotation from "blade up" (the held view's authored direction) to
			// the measured axis, clockwise-positive in screen space
			float rot = std::atan2(ax, -ay);

			// Held view pixels: material-retextured when present, raw otherwise
			std::vector<uint8_t> held;
			int16_t hw = 0, hh = 0;

			if (!shell_pixels(itemid, info, heldnode, false, held, hw, hh))
				return Texture();

			Point<int16_t> hgrip = heldnode["origin"];

			// Rotate the held image around its grip point (nearest sampling)
			float c = std::cos(rot);
			float s = std::sin(rot);
			...
			{
				float px = (corner & 1) ? float(hw) : 0.0f;
				float py = (corner & 2) ? float(hh) : 0.0f;
				float rx = c * (px - hgrip.x()) - s * (py - hgrip.y());
				float ry = s * (px - hgrip.x()) + c * (py - hgrip.y());
			...
			Point<int16_t> out_grip(int16_t(-min_x), int16_t(-min_y));
			...
			for (int16_t y = 0; y < out_h; ++y)
			{
				for (int16_t x = 0; x < out_w; ++x)
				{
					// Inverse-rotate back into held-image space
					float rx = float(x - out_grip.x());
					float ry = float(y - out_grip.y());
					int srcx = int(std::floor(c * rx + s * ry + hgrip.x()));
					int srcy = int(std::floor(-s * rx + c * ry + hgrip.y()));

					if (srcx < 0 || srcy < 0 || srcx >= hw || srcy >= hh)
						continue;

					size_t src_i = (size_t(srcy) * hw + srcx) * 4;
					size_t dst_i = (size_t(y) * out_w + x) * 4;

					out[dst_i] = held[src_i];
					out[dst_i + 1] = held[src_i + 1];
					out[dst_i + 2] = held[src_i + 2];
					out[dst_i + 3] = held[src_i + 3];
				}
			}

			return Texture::from_pixels(key, out_w, out_h, std::move(out), out_grip);
		}
